...
class InTransitCollisionResolver:
    ...
    def resolve(
        ...
        events = self._collect_collision_events(pending_moves, previous_time_ms, current_time_ms)
        if not events:
            return pending_moves
    ...
    def _collect_collision_events(
        self, pending_moves: list, previous_time_ms: int, current_time_ms: int
    ) -> list[tuple]:
        events: list[tuple] = []
    ...
    def _compute_collision_event(
        ...
        max_step_a = len(path_a) - 1
        max_step_b = len(path_b) - 1

        for step_a in range(1, max_step_a + 1):
            step_time_a = start_time_a + step_a * TRANSIT_DURATION_MS
            if step_time_a <= previous_time_ms or step_time_a > current_time_ms:
                continue
            for step_b in range(1, max_step_b + 1):
                step_time_b = start_time_b + step_b * TRANSIT_DURATION_MS
                if step_time_a != step_time_b:
                    continue
                if path_a[step_a] != path_b[step_b]:
                    continue
                stop_a = path_a[step_a - 1]
                stop_b = path_b[step_b - 1]
                return (step_time_a, move_a, move_b, stop_a, stop_b)
        return None
