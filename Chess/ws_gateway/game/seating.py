"""Assigns a seat in a session and sends the resulting role/state."""
...
async def seat_and_notify(session, connection) -> str | None:
    ...
    await connection.send_json({"type": "role_assigned", "role": _ROLE_NAMES[role], "room_id": session.session_id})
    await connection.send_json({"type": "state", "snapshot": snapshot_to_wire(session.get_viewer_snapshot(role))})
