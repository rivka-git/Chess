...
from core.engine.controller import Controller
...
from ws_gateway.game.click_handler import ClickHandler
from ws_gateway.game.disconnect_timer import DisconnectTimer
from ws_gateway.game.tick_loop import TickLoop
from ws_gateway.config import DISCONNECT_TIMEOUT_S
...
class GameSession:
    ...
    async def _notify_countdown(self, opponent_color: str, seconds_remaining: int) -> None:
        ...
        if opponent is None:
            return
        try:
            await opponent.send_json({"type": "opponent_left", "seconds_remaining": seconds_remaining})
        except Exception:
            pass
