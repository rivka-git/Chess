"""Unit tests for server.game.session_manager.SessionManager."""
...
def test_remove_unknown_id_does_not_raise():
    make_manager().remove("ghost")  # should not raise
...
def test_game_ended_for_unknown_room_does_not_raise():
    bus = EventBus()
    manager = SessionManager(bus)
    bus.publish(events.GAME_ENDED, {"room_id": "no-such-room"})  # should not raise
