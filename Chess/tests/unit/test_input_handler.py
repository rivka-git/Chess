...
def test_click_selects_own_piece_when_color_given():
    handler = make_handler()
    board = Board([["wK", "bK"]])
    handler.handle_click(board, 50, 50, lambda s, e: None, color="w")
...
def test_click_rejects_opponent_piece_when_color_given():
    handler = make_handler()
    board = Board([["wK", "bK"]])
    handler.handle_click(board, 50, 50, lambda s, e: None, color="b")
...
def test_color_scoped_selection_independent_per_color():
    ...
    board = Board([["wK", "bK"]])
    handler.handle_click(board, 50, 50, lambda s, e: None, color="w")
    handler.handle_click(board, 150, 50, lambda s, e: None, color="b")
    assert handler.selection_for("w") == (0, 0)
...
def test_second_click_own_color_still_requests_move():
    ...
    moves = []
    handler.handle_click(board, 50, 50, lambda s, e: None, color="w")
    handler.handle_click(board, 150, 50, lambda s, e: moves.append((s, e)), color="w")
...
def test_jump_allows_own_piece_when_color_given():
    ...
    board = Board([["wR", "bR"]])
    jumps = []
    handler.handle_jump(board, 50, 50, lambda p: jumps.append(p), color="w")
    assert jumps == [(0, 0)]
...
def test_jump_rejects_opponent_piece_when_color_given():
    ...
    board = Board([["wR", "bR"]])
    jumps = []
    handler.handle_jump(board, 150, 50, lambda p: jumps.append(p), color="w")
    assert jumps == []
